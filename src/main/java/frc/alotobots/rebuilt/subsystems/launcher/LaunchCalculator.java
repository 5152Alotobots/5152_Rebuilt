...
public class LaunchCalculator {
    ...
    /**
     * Returns the cached launching parameters for this loop cycle, computing them if not yet calculated.
     */
    public LaunchingParameters getParameters() {
        ...
        forwardEstimatedPose =
                forwardEstimatedPose.exp(
                        new Twist2d(
                                robotRelativeVelocity.vxMetersPerSecond * LAUNCH_CALCULATOR_SUBSYSTEM_DELAY.in(Seconds),
                                robotRelativeVelocity.vyMetersPerSecond * LAUNCH_CALCULATOR_SUBSYSTEM_DELAY.in(Seconds),
                                robotRelativeVelocity.omegaRadiansPerSecond * LAUNCH_CALCULATOR_SUBSYSTEM_DELAY.in(Seconds)));

        // Calculate distance from turret to target
        boolean shouldFlip;
        if (DriverStation.getAlliance().isEmpty()) {
            DriverStation.reportWarning("No alliance data available to calculate distance to turret target. Defaulting to blue.", false);
            shouldFlip = false;
        } else {
        ...
        Pose2d turretPosition = forwardEstimatedPose.transformBy(ROBOT_TO_TURRET.toTransform2d());
        Distance turretToTargetDistance = Meters.of(target.getDistance(turretPosition.getTranslation()));
        ...
        // The time of flight depends on distance, but distance depends on time of flight (circular dependency).
        // We break this by iteratively refining our guess, each iteration uses the previous time of flight
        // to estimate where the robot will be, giving a better distance, giving a better time of flight, etc.
        // We exit early once the distance change between iterations has converged.
        Distance previousDistance = turretToTargetDistance;
        for (int i = 0; i < 20; i++) {
            timeOfFlight = (Time) FUEL_TIME_OF_FLIGHT_MAP.get(lookaheadTurretToTargetDistance);
            Translation2d turretOffsetToLookahead = new Translation2d(
            ...
            lookaheadTurretToTargetDistance = Meters.of(target.getDistance(lookaheadPose.getTranslation()));

            // Exit early here if converged
            if (lookaheadTurretToTargetDistance.minus(previousDistance).abs(Meters) < CONVERGED_DISTANCE_ESTIMATE_THRESHOLD.in(Meters)) {
                break;
            }
            
            previousDistance = lookaheadTurretToTargetDistance;
        }
        
        // Calculate parameters accounted for imparted velocity
        Rotation2d turretAngle = target.minus(lookaheadPose.getTranslation()).getAngle();
        Angle deflectorAngle = (Angle) LAUNCHER_DEFLECTOR_ANGLE_MAP.get(lookaheadTurretToTargetDistance);
        ...
        AngularVelocity turretVelocity = RadiansPerSecond.of(
                turretAngleFilter.calculate(
                        turretAngle.minus(lastTurretAngle).getRadians() / LOOP_PERIOD.in(Seconds)));
        AngularVelocity deflectorVelocity = RadiansPerSecond.of(
                hoodAngleFilter.calculate((deflectorAngle.minus(lastHoodAngle)).in(Radians) / LOOP_PERIOD.in(Seconds)));
        
        lastTurretAngle = turretAngle;
        lastHoodAngle = deflectorAngle;


        AngularVelocity shooterVelocity = (AngularVelocity) LAUNCHER_SHOOTER_VELOCITY_MAP.get(lookaheadTurretToTargetDistance);
    ...
    /**
     * Invalidates the cached parameters, forcing recomputation on the next {@link #getParameters()} call.
