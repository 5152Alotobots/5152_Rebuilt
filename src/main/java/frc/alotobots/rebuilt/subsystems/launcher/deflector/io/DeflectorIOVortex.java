...
public class DeflectorIOVortex implements DeflectorIO {
  ...
  public DeflectorIOVortex() {
    ...
    deflectorMotorConfig.idleMode(IdleMode.kCoast)
        .inverted(true);

    deflectorMotorConfig.encoder
    ...
    deflectorMotorConfig.closedLoop
        .pid(
    ...
    deflectorMotor.configure(deflectorMotorConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
  ...
  public void updateInputs(DeflectorIOInputs inputs) {
    ...
    inputs.deflectorMotorConnected = deflectorMotorConnectedDebouncer.calculate(
    ...
    inputs.deflectorPosition = Rotations.of(inputs.deflectorMotorPosition.in(Rotations) / DeflectorVortexConstants.SENSOR_TO_MECHANISM_RATIO);
    
    inputs.deflectorMotorVolts = Volts.of(deflectorMotor.getAppliedOutput() * deflectorMotor.getBusVoltage());
  ...
  public void setDeflectorPosition(Angle position, PIDSlots pidSlot) {
    ClosedLoopSlot slot = pidSlot == PIDSlots.VELOCITY ? ClosedLoopSlot.kSlot1 : ClosedLoopSlot.kSlot0;
    
    deflectorMotor.getClosedLoopController().setReference(
