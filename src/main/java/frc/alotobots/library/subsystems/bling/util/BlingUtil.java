...
public class BlingUtil {
    ...
    return new Trigger(
            () -> {
              ...
              if (DriverStation.isFMSAttached()) {
                ...
                currentMatchTime = Seconds.of(matchTimeSeconds);
              } else {
                // Not connected to FMS - calculate remaining time based on elapsed teleop time
                double elapsedTime = teleopTimer.get();
                double remainingTime = totalMatchTime.in(Seconds) - elapsedTime;
                currentMatchTime = Seconds.of(remainingTime);
              }
  ...
  public static Trigger scheduleAtMatchTime(Command command, Time timeRemaining) {
    return scheduleAtMatchTime(
        command, timeRemaining, Seconds.of(0.1), Seconds.of(135) // Standard teleop duration
        );
  }
}
